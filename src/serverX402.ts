...
app.listen(PORT, () => {
  ...
  //
  // ЗАЧЕМ. Листинг отклонён с формулировкой «не интегрирован официальный OKX
  // Payment SDK». Здесь стоит локальный фасилитатор, и причина выбора записана
  // в его шапке 26.07: облачный `OKXFacilitatorClient` требует ключей, а портал
  // web3.okx.com «недоступен по сети из Алматы».
  //
  // ЧЕГО НЕ ПРОВЕРИЛИ ТОГДА. Решение принято по доступности С МОЕЙ МАШИНЫ. Но код
  // работает ЗДЕСЬ, и ревьюер OKX стучится СЮДА. «Я не вижу домен» и «сервис не
  // видит домен» — разные утверждения, и второе никто не измерял.
  //
  // Проба ничего не ломает: включается переменной, пишет в лог, не блокирует старт.
